...
// their own key. Plain on purpose; the look arrives with #6. What it must get right:
...
//   - the key is typed into a hidden box and never shown again, only its last four characters,
//     so it can't end up in a screenshot or a screen recording;
//   - when photoreal is showing, it says the imagery's shadows are illustrative (PLAN.md D4);
//   - when the imagery can't be had, it says why in words a runner can act on.
import { html, link } from "../dom";
import { KEY_PROVIDERS, type KeyProvider, type OwnKey, recognizeKey } from "./key";
import type { PhotorealState, Problem } from "./photoreal";
...
export interface PhotorealActions {
...
const CAMERA_HEIGHT_CAVEAT = "Measured from open terrain data, not from the imagery. On a bridge it counts from the ground or water underneath.";

export function createPhotorealPanel(container: HTMLElement, actions: PhotorealActions): PhotorealPanel {
  let state: PhotorealState | undefined;

  // On the map.
  const main = html("button", { type: "button", class: "photoreal-main" });
  const settings = html("button", { type: "button", class: "photoreal-settings", text: "Key" });
  const status = html("p", { class: "photoreal-status", role: "status" });
  ...
  const box = html("input", { type: "password", id: "photoreal-key", autocomplete: "off", spellcheck: "false", size: 36, "aria-describedby": "photoreal-recognized photoreal-error" });
  ...
  container.replaceChildren(html("div", { class: "photoreal-control" }, main, settings), status, cameraHeight, dialog);

  main.addEventListener("click", () => {
    if (!state?.key) openPanel();
    else if (state.look === "photoreal") actions.turnOff();
    else actions.turnOn();
  });
  ...
  function showRecognized(): void {
    error.textContent = "";
    const key = recognizeKey(box.value);
    recognized.textContent = key ? `That is a ${KEY_PROVIDERS[key.provider].keyName}. It will be sent to ${KEY_PROVIDERS[key.provider].company} (${KEY_PROVIDERS[key.provider].sentTo}) and nowhere else.` : "";
  }
  ...
  return {
    show(next) {
      ...
      main.disabled = look === "loading";
      main.setAttribute("aria-pressed", String(look === "photoreal"));
      settings.hidden = key === null; // with no key, the main button opens the panel itself
      settings.setAttribute("aria-label", "Photoreal key settings");
...
function problemInWords(problem: Problem, provider: KeyProvider): string {
  const { company, keyName } = KEY_PROVIDERS[provider];
  switch (problem) {
    case "refused":
      return provider === "google"
        ? "Google refused this key, so you are on the plain map. Check that the key was copied whole, that its project has billing switched on, and that the Map Tiles API is enabled for it. Then press “Make it photoreal” again."
        : "Cesium refused this token, so you are on the plain map. Check that the token was copied whole, and that “Google Photorealistic 3D Tiles” is among My Assets in your ion account (it can be added from ion's Asset Depot). Then press “Make it photoreal” again.";
    case "over-limit":
      return `${company} says this ${keyName} has been used too much for now, so you are on the plain map. The allowance resets with time; ${company}'s own dashboard shows how much is left.`;
    case "unreachable":
...
function keyHelp(provider: KeyProvider): HTMLElement {
  ...
  const steps = link(help.stepsSource.source, "Source");
  steps.title = `Checked on ${help.stepsSource.accessed}`;
  return html(
    "section",
    ...
    html("p", { class: "plan-fact" }, "Steps: ", steps),
  );
}
