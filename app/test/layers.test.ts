...
/** Sun, the second layer there really is: built from the course and from one runner's plan (#9). */
const sun = sunLayer(nyc, createPlanner(plannerCourse(nyc), defaultPlan(plannerCourse(nyc))))!;
...
describe("the layer switches", () => {
  ...
  it("only exist for layers the course has: no buildings, no Sun switch", () => {
    ...
    expect(layersFor(nyc).map((layer) => layer.id)).toEqual(["hills", "sun"]);

    function layersFor(bundle: typeof nyc): Layer[] {
      return [hillsLayer(bundle), sunLayer(bundle, createPlanner(plannerCourse(bundle), defaultPlan(course)))].filter((layer) => layer !== null);
    }
  });
...
describe("what a layer puts on screen", () => {
  const layers = [hillsLayer(nyc), sun, wind];
  ...
  it("shows only the layer that is on", () => {
    ...
    expect(screen.rows.map((row) => row.name)).toEqual(["Sun", "All-day sun"]);
  ...
  it("opens every layer's rows with Show everything, without marking the map or lengthening the sentence", () => {
    ...
    expect(screen.rows.map((row) => row.name)).toEqual(["Grade", "Effort", "Sun", "All-day sun"]);
    expect(screen.lineMarks).toEqual([]);
    expect(screen.clause(24.5, "km")).toBeNull();
  });
});
