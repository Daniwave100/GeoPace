...
| D19 | ✅ **Course Bundle = one JSON file per course**, `data/derived/<id>/course-bundle.json`, validated on both sides against `schema/course-bundle.schema.json` (`schema_version` 1; 2 since D38 added editions; 3 since D45 added where the height is not measured). Measured data (and values computed from it by published models, like difficulty) sits under `measured`; subjective data will get its own section. Every landmark carries its source into the bundle. | One versioned contract; drift is caught when loading instead of showing wrong numbers. Berlin's bundle is ~210 KB. | 09-16 |
...
| Bridge deck heights | n/a — decks spanned straight (D18) | ✅ [2017 NYC Topobathymetric LiDAR](https://www.fisheries.noaa.gov/inport/item/64728), class 17 (bridge deck), as COPC tiles on NOAA Digital Coast; only the corridor around each bridge is read (D23). ⚠️ No returns at all over the middle of the Verrazzano's main span (km 0.77–1.36): that stretch is a straight line between measured deck heights, so the real crest is a few meters higher. The bundle lists it, and four shorter gaps, as not measured (D45). | NYC Open Data / NOAA, as above | ✅ NYC |
...
- **Palette.** Light: paper `#f4f4f0`, ink `#000`, blue `#1546ff`, grey `#8a8a86`. Dark: the inverse — ground `#000`,
  ...
  Grey *words* are darker than grey marks so they stay readable (D49).
...
- 🟡 **The owner's look at Explore (#6)**, on the keyless map and over photoreal imagery with their own key. What to
  ...
  bridge a few dozen metres long deserves its grey dashes. Also Claude's calls, the owner's to overrule: nothing is on
  when the app opens; the layer switches sit on the strip rather than on the map; the race plan opens from the banner;
  no dead "Ride the course" button until #8 builds it.
