...
6. 🟡 **The course line on the road** (#22), *built; the look is still the owner's to settle*: in photoreal the line is drawn at the road's own height, from the pipeline's
   ...
   🟡 *Still to do, and only the owner can:* settle the look by eye on the throwaway branch `prototype/line-on-road` (§10).
...
- 🟡 **The owner's look at the line on the road (#22, D52), with their own key.** Only the owner can: `git checkout
  prototype/line-on-road`, run the app, open `http://localhost:5173/?course=nyc&line-trial`, turn photoreal on. Go to the
  Queensboro, the Verrazzano and Central Park (and, on Berlin, the Tiergarten), and move the camera round the line from
  above and from near road height. To judge: **does the line stay on the road as the camera moves** (switch "Draw the
  line" to *Draped (old)* to see the difference); the **smallest lift** that stays clean on both courses; what should be
  left of the line **behind a tree, a tower, a building** (hidden, fainter, same as ever) and how faint; on the
  Verrazzano's **unmeasured main span**, where the line is a few metres under the real deck and is never hidden, whether
  fainter or full strength reads better (and the same on the Queensboro's two short unmeasured stretches, which are under
  the upper deck; and note that with no layer on, such a stretch is plain blue, as it is on the keyless map); and whether the **not-measured dashes** now read the same over dark imagery as
  over the pale map. The panel writes the picks in one line to copy back; they go into `ROAD_LOOK` and D52, and the
  prototype branch is then deleted.
