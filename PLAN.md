...
- 🟡 **The owner's look at the line on the road (#22, D52), with their own key.** Only the owner can: `git checkout
  ...
  left of the line **behind a tree, a tower, a building** (hidden, fainter, same as ever) and how faint; the same on the
  Verrazzano's **unmeasured main span**; and whether the **not-measured dashes** now read the same over dark imagery as
...
- **2026-09-18** — The course line on the road (#22). **Pipeline:** every course-line point now carries its height above
  ...
  throwaway branch `prototype/line-on-road` (§10). #8 is unblocked: the road's height is there for its camera.
