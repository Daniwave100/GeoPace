...
- **2026-09-20** — **The Sun layer, settled before a line of it is written** (D58, #9, #10). The owner asked to be grilled
  ...
  owner chose to use it anyway, so the screen has to say so, and the error at least runs the safe way (a missed shade
  source reads as sun). And **no third-party shade source is usable**: ShadeMap is proprietary and sold by the square
