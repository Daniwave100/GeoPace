...
- 🔍 **NYC 2026 date and wave times, by a person**: nyrr.org sits behind a waiting room that a browser passes and our
  tools don't (and we don't work around it). Open it, confirm the date and the five wave times, then update
  `data/courses/nyc/editions/2026.yaml`: set the date's `confirmed: true`, and drop `carried_over` from what is confirmed (D38).
- 🟡 **Three calls from #5 for the owner to confirm**: Berlin's waves 2–6 shown as "start time not published" instead of
  carried over (D38a); NYRR's 2025 runner guide on `webassets.nyrr.org` as the source for NYC (D38b); the map dimming
  with the sun (D39). Each is easy to reverse: two YAML files and three lines of `globe.ts`.
