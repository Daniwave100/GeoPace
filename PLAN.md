...
| D10 | ✅ **Launch target ≈ 2026-10-18**, two weeks before the NYC Marathon (Nov 1, from the organizer's first-Sunday-in-November rule — D38; NYRR's 2026 page still to be read by a person). Repo private until the flyover + shade demo works, then public with a 30–60s photoreal clip. Owner posts; Claude drafts. Name stays GeoPace. | NYC is inside its 2–8 week research window now; Berlin 2026 (Sept 27) is too close. | 09-16 |
...
| D38 | ✅ **Edition facts are one YAML file per year, `data/courses/<id>/editions/<year>.yaml`, and travel in the Course Bundle (`schema_version` 2).** Each holds the race date and the runner waves, every fact with its source. A wave time is written as the organizer prints it (wall clock, in quotes) and the pipeline adds the exact instant with its UTC offset, worked out in the course's IANA zone. Two honesty rules: a time copied from an earlier edition is marked **carried over**, with the edition it came from and a reason the runner sees, and the app greys every time of day that rests on it; a wave whose time **nobody has published** is listed with no time and a note, can't be picked, and is never filled with a guess. ⛔ Third-party wave times: not an organizer or an authority (`CONTEXT.md`, *Sourced fact*). | Berlin 2026: the organizer confirms the date and only the first runner start (08:45, "in 6 waves"); the other five are on each runner's start card. NYC 2026: nyrr.org sat behind a waiting room our tools can't pass and we don't work around, so the five wave times are carried over from NYRR's own 2025 runner guide, and the date comes from its rule "the first Sunday in November". A required new field breaks old readers, hence version 2. | 09-18 |
| D39 | ✅ **The 3D scene has no clock of its own: Cesium's clock is stopped and set from the Planner's race clock**, so the scene's sun is the sun at the moment the runner reaches that km. Cesium's sun lighting is kept on at city scale (by default it fades out within ~10,000 km of the ground, so a city never looks dark). The map therefore dims as the sun gets low and drops to Cesium's night-side floor (30%) after sunset. *(Accepted by default; owner can revisit on seeing it — it is two lines in `app/src/scene/globe.ts`.)* | One source of time means the strip, the readout, the marker and the sun can't drift apart. Without the lighting change the sun moved but nothing on screen showed it: measured in headless Chrome, map brightness 206 → 63 between an 11:30 start and an 18:00 finish in New York on Nov 1. It becomes properly visible with the white model's shadows (#7). | 09-18 |
...
| Race date / start waves | ✅ Sunday 2026-09-27; runners start "from 08.45 am … in 6 waves" after the handbike and wheelchair starts (8.20, 8.26, 8.29) — [race day page](https://www.bmw-berlin-marathon.com/en/your-race/race-day-for-runners), accessed 2026-09-18. ⚠️ The organizer publishes **no clock time for waves 2–6** (checked in the page's HTML on 09-18; each runner's time is on their emailed start card), so they are listed without one (D38). Third-party sites print times; one credits the organizer's page, which doesn't contain them. | ✅ Date 2026-11-01, from NYRR's rule "first Sunday in November" ([2025 runner guide](https://webassets.nyrr.org/nyrrwebsiteassets/TCSNYCM25_RunnerGuide_Mobile_M.pdf); [Abbott WMM](https://www.worldmarathonmajors.com/races/new-york-city) says the same). Waves 9:10 · 9:45 · 10:20 · 10:55 · 11:30 are **carried over from 2025** (same guide). 🔍 nyrr.org was behind its waiting room again on 2026-09-18: a person needs to open it in a browser and confirm the 2026 date and waves. | Facts, paraphrased with a link | ✅ Berlin · 🔍 NYC |
...
- 🔍 **NYC 2026 wave times, by a person**: nyrr.org sits behind a waiting room that a browser passes and our tools don't
  (and we don't work around it). Open it, confirm the date and the five wave times, then update
  `data/courses/nyc/editions/2026.yaml` and drop `carried_over` from what is confirmed (D38).
- 🔍 **Berlin 2026 waves 2–6** have no published clock time; fill them in if the organizer publishes them. Berlin's race
  is before launch, so this matters mainly as the worked example of "not published".
- 🟡 **A runner can't type their own start time.** It would cover every wave nobody has published (the time is on their
  start card). Claude's suggestion, not built: it adds to the Race Plan, so it is the owner's call.
- 🟡 **The strip and readout count km along the course line** (NYC ends at 42.69), not organizer km. D20 already says to
  revisit this in #12; a runner will expect the finish to read 42.2.
...
- **2026-09-18** — Race plan, race clock and scrubbing (#5): edition facts for both courses and the carried-over and
  ...
  an organizer source; its wave times are carried over from 2025 until a person can read nyrr.org. The mockups use the
  same edition facts instead of invented wave times.
