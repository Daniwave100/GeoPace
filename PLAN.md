...
- **CesiumJS draws no shadow further from the camera than `shadowMap.maximumDistance`.** Set under the height the
  ...
  evening. A test pins every quality's distance above the Ride's own camera height (D56).
...
- 🟡 **The owner's first look at the White model (#7, D56)**, owed, and the only judgement that matters on it. What
  ...
  **after sunset**, where the ground goes dark and the blocks stay pale, so the city is brighter than the street it
  ...
  all**, because the whole course is seen from 25 to 43 km up and shadows are drawn within 6 to 12 km of the camera —
