...
6. 🟡 **The course line on the road** (#22), *built, and the owner has confirmed the line and picked fainter-behind-things; a second look is owed at three fixes (§10)*: in photoreal the line is drawn at the road's own height, from the pipeline's
...
- 🟡 **The owner's second look (#22), because only real imagery can show it.** Three things changed after the first
  look and nobody has seen them over Google's city: the **flat grey band** where the height is not measured, in place
  of the "weird rectangles" (D47: on the Verrazzano's main span and the Queensboro's two short gaps, with Hills on);
  the line being **one strength across its whole width** instead of solid above and faded below (D52), and whether
  "behind something" starts and ends at the right distance (it should stay solid on an open road and on a bridge deck,
  and fade under trees, under the Queensboro's upper deck and behind buildings); and the **runner's dot**, now always
  on top of the line. `git checkout prototype/line-on-road` still has the trial panel (`?line-trial`) for the lift and
  the faintness; if the defaults are what you want, `v1-foundation` has them. When this is settled the prototype
  branch is deleted.
